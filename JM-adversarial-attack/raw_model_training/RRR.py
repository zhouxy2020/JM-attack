...
def downsample_basic_block(x, planes):
    ...
    # zero_pads = torch.tensor((x.size(0), planes - x.size(1),  x.size(2))).zero_()
    zero_pads = torch.randn((x.size(0), planes - x.size(1), x.size(2))).zero_()
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        funcs = []
        ##？，2, 128
        x = self.conv1(x) #？，16,128
        x = self.bn1(x)
        x = self.relu(x)    #

        x = self.layer1(x)  #  #？，16,128
