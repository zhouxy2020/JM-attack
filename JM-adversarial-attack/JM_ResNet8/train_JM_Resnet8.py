...
def adjusted_cosine_similarity(v1, v2, eps=1e-8):
    ...
    norm_x1 = torch.mean(x1, dim=0, keepdim=True)
    norm_x2 = torch.mean(x2, dim=0, keepdim=True)
    X1_m = x1 - norm_x1.repeat(x1.shape[0],1)
    X2_m = x2 - norm_x2.repeat(x2.shape[0],1)
    R_num = torch.matmul(X1_m, X2_m.T).diag()
    X1_norm = torch.norm(X1_m, p=2, dim=1)
    X2_norm = torch.norm(X2_m, p=2, dim=1)
...
def pearson_correlation_coefficient(v1, v2):
    x1 = v1.reshape(v1.shape[0], -1)
    x2 = v2.reshape(v2.shape[0], -1)
    norm_x1 = torch.mean(x1, dim=1, keepdim=True)
    norm_x2 = torch.mean(x2, dim=1, keepdim=True)
    X1_m = x1 - norm_x1.repeat(1,x1.shape[1])
    X2_m = x2 - norm_x2.repeat(1,x2.shape[1])
    R_num = torch.matmul(X1_m, X2_m.T).diag()
    X1_norm = torch.norm(X1_m, p=2, dim=1)
    X2_norm = torch.norm(X2_m, p=2, dim=1)
    R_den = X1_norm * X2_norm
...
def train_model(x, Epsilon_uni, args, batch_signal, model, optimizer,  scheduler, num_epochs=50):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in pbar:
            ...
            # pearsonr_loss = pearsonr(feature_ori, feature_adv)
            pearsonr_loss = pearson_correlation_coefficient(feature_ori, feature_adv)
            adjusted_cos_loss = adjusted_cosine_similarity(feature_ori, feature_adv)
            cos_loss = cosine_similarity(feature_ori, feature_adv)
            loss = (pearsonr_loss+adjusted_cos_loss).mean()
            # loss = torch.add(pearsonr_loss.squeeze(),adjusted_cos_loss).mean()
            loss.backward()
            optimizer.step()
            # statistics
        ...
        if best_loss > epoch_loss:
            ...
            best_x = x
            print('epoch:{}, JM best loss:{}'.format(epoch, best_loss))
    # 保存训练精度最高时的扰动
    with open(args.model+'_JM_SNR10_PSR-10.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
...
def load_model(model):
    ...
    model_ft.load_state_dict(
        torch.load("../raw_model_training/result/model/"+model+"_best_lr=0.0001.pth"))
    model_ft.eval()
    return model_ft
