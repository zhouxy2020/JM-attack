...
from Utils.dataset import get_signal_train_validate_loader, get_signal_test_loader, ModCls_loaddata
...
def main():
    ...
    if dataset == '128' or '512' or '1024' or '3040':
        ...
        model_location = '../DefenseEnhancedModels/PAT/128_CNN2Donly_eps0.0006/model_best.pth_epoch199.tar'
    ...
    with torch.no_grad():
        for icn in range(len(PSR_vec)):
            ...
            for rnd in range(UAP_pearacos_all.shape[0]):
                ...
                for batch_counter in range(test_X_0.shape[0] // batchsize):
                    ...
                    test_label = torch.argmax(test_label, dim=1).to(device)

                    test_jamming = torch.from_numpy(x_batch_test+jamming)
                    test_jamming = test_jamming.type(torch.FloatTensor).to(device)
                    ...
                    test_pearacos = test_pearacos.type(torch.FloatTensor).to(device)
                    ...
                    test_fg = test_fg.type(torch.FloatTensor).to(device)
                    ...
                    test_deepfool = test_deepfool.type(torch.FloatTensor).to(device)
                    ...
                    test_pca = test_pca.type(torch.FloatTensor).to(device)
        ...
        for batch_counter in range(test_X_0.shape[0] // batchsize):
            ...
            test_clean = test_clean.type(torch.FloatTensor).to(device)
