...
class PGD():
    ...
    def attack(self, epsilon=0.1, alpha=0.005, emb_name='emb.', is_first_attack=False):
...
class PATDefense(Defense):
    ...
    def pgd_generation(self, var_natural_images=None, var_natural_labels=None):
        ...
        for i in range(self.attack_step_num):
            ...
            # loss.backward()
            # gradient = var_copy_images.grad.detach()
            # gradient_sign = gradient.sign().cpu().numpy()
    ...
    def train_one_epoch_with_pgd_and_nat(self, train_loader, epoch):
        ...
        #     logits_nat = self.model(nat_images)
        #     loss_nat = F.cross_entropy(logits_nat, nat_labels)
        #     loss_nat.backward()  # 反向传播，得到正常的grad
        #     pgd.backup_grad()
        #     # 对抗训练
        #     for t in range(self.attack_step_num):
        #         pgd.attack(is_first_attack=(t == 0))  # 在embedding上添加对抗扰动, first attack时备份param.data
        #         if t != self.attack_step_num - 1:
        #             model.zero_grad()
        #         else:
        #             pgd.restore_grad()
        #         logits_adv = self.model(nat_images)
        #         loss_adv = F.cross_entropy(logits_adv, nat_labels)
        ...
        #
        #     print('\rTrain Epoch {:>3}: [{:>5}/{:>5}]  \t total_loss={:.4f} ===> '
        #           .format(epoch, (index + 1) * len(images), len(train_loader) * len(images), loss_adv),
        #           end=' ')  # loss_adv={:.4f},loss_nat,loss_adv,loss_nat={:.4f},
