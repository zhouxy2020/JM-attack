...
def train_model(x, Epsilon_uni, args, batch_signal,  model , optimizer,  num_epochs=50):
    best_loss = 100.0
    for epoch in range(num_epochs):
        ...
        for inputs, labels in pbar:
            ...
            loss = 1-torch.mean(torch.abs(feature_ori-feature_adv))
            # loss = norm_loss.mean()
            loss.backward()
            optimizer.step()
            # statistics
        ...
        if best_loss > epoch_loss:
            ...
            best_x = x
            print('epoch:{}, Manhattan best loss:{}'.format(epoch, best_loss))
    # 保存训练精度最高时的扰动
    with open(args.model+'_Manhattan_UAP.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
...
def main():
    ...
    x = Variable(torch.distributions.uniform.Uniform(low=-0.001, high=0.001).sample(sample_shape=([1, 2, 128]))).cuda()
    x.requires_grad = True
    # 优化器
    optimizer_ft = optim.Adam([x], lr=prog_args.lr)
    # 训练模型
    train_model(x, Epsilon_uni, prog_args, batch_signal, model_ft, optimizer_ft , num_epochs=prog_args.num_epochs)
