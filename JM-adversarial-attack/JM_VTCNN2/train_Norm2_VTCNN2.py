...
def train_model(x, Epsilon_uni, args, batch_signal,  model , optimizer,  num_epochs=50):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in pbar:
            ...
            loss = 1-torch.norm(feature_ori-feature_adv)
            # loss = norm_loss.mean()
            loss.backward()
            optimizer.step()
            # statistics
        ...
        if best_loss > epoch_loss:
            ...
            best_x = x
            print('epoch:{}, Norm2 best loss:{}'.format(epoch, best_loss))
    # 保存训练精度最高时的扰动
    with open(args.model+'_Norm2_UAP.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
...
def main():
    ...
    x = Variable(Epsilon_uni * torch.div(xx, torch.norm(xx) + 0.00001)).cuda()
    # x = Variable(torch.distributions.uniform.Uniform(low=-Epsilon_uni, high=Epsilon_uni).sample(sample_shape=([1, 2, 128]))).cuda()
